...
def get_patients() -> list[object]:
    try:
        conn = __connect(os.getenv('DB_PATH'))
        cur = conn.cursor()
        cur.execute('SELECT id, fullName, teeth, actions, price, comment, date FROM patients ORDER BY id DESC')
...
def get_patients_by_full_name(full_name) -> list[object]:
    try:
        conn = __connect(os.getenv('DB_PATH'))
        cur = conn.cursor()
        cur.execute(f'SELECT id, fullName, teeth, actions, price, comment, date FROM patients WHERE fullName LIKE \'%{full_name}%\' ORDER BY id DESC')
        patients = __convert(cur.fetchall())
    except (Exception) as error:
        print(error)
    finally:
...
def get_patient(id) -> object:
    try:
        conn = __connect(os.getenv('DB_PATH'))
        cur = conn.cursor()
        cur.execute(f'SELECT id, fullName, teeth, actions, price, comment, date FROM patients WHERE id = {id} ORDER BY id DESC')
